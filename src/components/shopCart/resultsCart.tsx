	...
	return (
		<div className={s.empty}>
			<div className={s.empty__inner}>
				...
				<h4 className={`${s.empty__title} ${isGreen ? s.empty__title__green : ""}`}>{title}</h4>
				<p className={s.empty__text}>{text}</p>
				<button onClick={backTo} className={`${s.empty__btn} btn`}>
					<img src="/svg/arrow.svg" /> Go back
				</button>
			</div>
		</div>
	)
